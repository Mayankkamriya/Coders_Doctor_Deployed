    ...
    <div className="p-6">
      ...
      <div className="flex gap-4 overflow-x-auto p-2">
        {Object.keys(availableSlots).map((date) => (
          <button
            key={date}
            className={`px-4 py-2 border rounded ${selectedDate === date ? "bg-blue-500 text-white" : "bg-gray-200"}`}
      ...
      {selectedDate && (
        <div className="flex gap-4 overflow-x-auto p-2 mt-4">
          {availableSlots[selectedDate].map((time, index) => (
            <button
              key={index}
              className={`px-4 py-2 border rounded ${selectedTime === time ? "bg-green-500 text-white" : "bg-gray-200"}`}
